// src/hooks/employes/exportHelpers.ts
import type { ExportPeriod } from './types';

// ⭐ Fikajiana ny date range ho an'ny export (local date — tsy UTC)
export const getExportRange = (period: ExportPeriod, customDate: string) => {
  const now = new Date();
