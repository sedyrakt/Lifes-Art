...
const MINIATURE_DARK_PATH = './images/licensedark.jpeg';
const MINIATURE_LIGHT_PATH = './images/licenselight.jpeg';
...
const FormInput: React.FC<{
  ...
  const borderColor = hasError ? '#FCA5A5' : focused ? '#4F46E5' : isDark ? 'rgba(255,255,255,0.15)' : '#CBD5E1';
  ...
  return (
    <div className="min-w-0">
      {label && (
        <label htmlFor={name} className="mb-1 block text-[14px] font-medium leading-tight" style={{ color: mutedColor }}>
          {label}
        </label>
      )}
      <div className="relative">
        ...
        <input
          ...
          className="h-10 w-full rounded-md border pl-9 pr-3 text-[14px] font-medium leading-tight outline-none transition-all disabled:cursor-not-allowed disabled:opacity-60"
          style={{
            background: inputBg,
            color: textColor,
            borderColor,
      ...
      {error && <p className="mt-1 text-[13px] font-medium leading-tight text-red-500">{error}</p>}
    </div>
  );
};
    ...
    <div className={`flex min-h-screen w-full items-center justify-center p-3 ${isDark ? 'bg-[#0F172A]' : 'bg-white'}`}>
      <div className="flex w-full max-w-[1000px] overflow-hidden rounded-xl border" style={{ background: isDark ? '#0F172A' : '#FFFFFF', borderColor, boxShadow: isDark ? '0 20px 50px -12px rgba(0,0,0,0.6)' : '0 20px 50px -12px rgba(79,70,229,0.25)' }}>
        <div className="hidden w-1/2 flex-col justify-between border-r p-5 lg:flex" style={{ background: isDark ? '#1E293B' : '#EEF2FF', borderColor }}>
          <div>
            <div className="mb-3 flex items-center gap-2"><Skeleton className="h-7 w-7 rounded-lg" /><Skeleton className="h-3 w-16" /></div>
        ...
        <div className="flex w-full flex-col justify-center p-5 lg:w-1/2">
          <div className="space-y-3"><Skeleton className="h-6 w-32" /><Skeleton className="h-8 w-full" /><Skeleton className="h-8 w-full" /><Skeleton className="h-3 w-24" /><Skeleton className="h-8 w-full" /></div>
        </div>
      </div>
    </div>
  );
    ...
    return (
      <div className="relative flex min-h-screen w-full items-center justify-center p-3 bg-cover bg-center" style={{ backgroundImage, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <button
          type="button"
          onClick={toggleTheme}
          className="fixed right-4 top-4 z-40 flex h-8 w-8 items-center justify-center rounded-full border transition-all hover:scale-105"
        ...
        <div className="relative z-10 flex w-full max-w-[1000px] overflow-hidden rounded-xl border" style={{ background: surface, borderColor, boxShadow: shadow }}>
          ...
          <div className="hidden w-1/2 flex-col justify-between border-r p-5 lg:flex" style={{ background: surface, borderColor }}>
            <div className="space-y-3">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg" style={{ background: 'linear-gradient(135deg, #4F46E5, #4338CA)', boxShadow: `0 2px 8px ${primaryBg}` }}>
                    <Cloud className="h-5 w-5 text-white" />
                  </div>
                  <div>
                    <h1 className="text-[16px] font-bold leading-tight" style={{ color: textColor }}>Life's Art ERP</h1>
                    <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Enterprise Solution</p>
                  </div>
                </div>
                <div>
                  <h2 className="text-[20px] font-bold leading-tight" style={{ color: textColor }}>Licence<br />Active</h2>
                  <p className="mt-1 text-[13px] leading-snug" style={{ color: mutedColor }}>Votre licence est valide et prête à l'utilisation</p>
                </div>
              </div>
              ...
              <div className="space-y-1">
                {licenseFeatures.map((text, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <ShieldCheck size={16} className="mt-0.5 shrink-0" style={{ color: licenseBadges[i % 4].color }} />
                    <span className="text-[14px] leading-snug" style={{ color: mutedColor }}>{text}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-1.5 mt-3">
              {licenseBadges.map((item, i) => (
                <div key={i} className="flex items-center gap-1.5 rounded-lg border px-2.5 py-2" style={{ borderColor }}>
                  <item.icon size={13} style={{ color: item.color }} />
                  <span className="text-[13px] font-medium leading-tight truncate" style={{ color: textColor }}>{item.title}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* RIGHT PANEL - FOTSY */}
          <div className="flex w-full flex-col gap-3 p-5 lg:w-1/2" style={{ background: rightBoxBg }}>
            <div className="flex items-center gap-2 lg:hidden">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ background: 'linear-gradient(135deg, #4F46E5, #4338CA)' }}>
                <Cloud className="h-4 w-4 text-white" />
              </div>
              <span className="text-[15px] font-bold leading-tight" style={{ color: textColor }}>Life's Art</span>
            </div>

            {/* ⭐ LOGO + TITRE — tsy misy mt-5 intsony */}
            <div className="flex flex-col items-center gap-1.5">
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-[14px]" style={{ color: mutedColor }}>Votre licence est valide</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between rounded-lg border p-2.5" style={{ borderColor, background: surfaceAlt }}>
                <div>
                  <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Code d'activation</p>
                  <p className="font-mono text-[14px] font-semibold leading-tight" style={{ color: textColor }}>{currentLicenseKey || 'N/A'}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-1.5">
                <div className="rounded-lg border p-2.5" style={{ borderColor }}>
                  <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Plan</p>
                  <p className="text-[14px] font-semibold capitalize leading-tight" style={{ color: '#4F46E5' }}>{packageName || 'Non spécifié'}</p>
                </div>
                <div className="rounded-lg border p-2.5" style={{ borderColor }}>
                  <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Expire le</p>
                  <p className="text-[14px] font-semibold leading-tight" style={{ color: textColor }}>{isLifetime ? 'Illimitée' : formattedExpiry}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-1.5">
                <div className="rounded-lg border p-2 text-center" style={{ borderColor }}>
                  <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Type</p>
                  <p className="text-[13px] font-semibold leading-tight" style={{ color: textColor }}>{isTest ? 'Test' : isLifetime ? 'Lifetime' : 'Standard'}</p>
                </div>
                <div className="rounded-lg border p-2 text-center" style={{ borderColor }}>
                  <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Jours restants</p>
                  <p className="text-[13px] font-semibold leading-tight" style={{ color: textColor }}>{isLifetime ? '∞' : daysRemaining}</p>
                </div>
                <div className="rounded-lg border p-2 text-center" style={{ borderColor }}>
                  <p className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Statut</p>
                  <p className="text-[13px] font-semibold leading-tight" style={{ color: '#10B981' }}>Active</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <button
                ...
                className="group flex h-10 w-full items-center justify-center gap-2 rounded-lg text-[15px] font-semibold text-white transition-all hover:opacity-95 bg-brand-500 hover:bg-brand-600"
              ...
              <button
                ...
                className="flex h-10 w-full items-center justify-center gap-2 rounded-lg text-[15px] font-medium transition-all hover:opacity-80"
            ...
            <div className="flex items-center justify-between border-t pt-2 mt-auto" style={{ borderColor }}>
    ...
    <div className="relative flex min-h-screen w-full items-center justify-center p-3 bg-cover bg-center" style={{ backgroundImage, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center backdrop-blur-sm" style={{ background: isDark ? 'rgba(15,23,42,0.9)' : 'rgba(255,255,255,0.9)' }}>
          <div className="w-[340px] max-w-[calc(100vw-32px)] rounded-2xl border p-6 text-center" style={{ background: surface, borderColor, boxShadow: shadow }}>
            <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-xl" style={{ background: primaryBg }}>
              <Loader2 className="h-5 w-5 animate-spin text-brand-500" />
            </div>
            <div className="text-[15px] font-semibold leading-tight" style={{ color: textColor }}>Activation en cours</div>
            <div className="mt-1 text-[14px] leading-snug" style={{ color: mutedColor }}>Vérification de votre licence...</div>
          </div>
        </div>
      )}
      ...
      <div className="relative z-10 flex w-full max-w-[1000px] overflow-hidden rounded-xl border" style={{ background: surface, borderColor, boxShadow: shadow }}>
        ...
        <div className="hidden w-1/2 flex-col justify-between border-r p-5 lg:flex" style={{ background: surface, borderColor }}>
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <div>
                <h2 className="text-[20px] font-bold leading-tight" style={{ color: textColor }}>
                  {isExpired ? 'Licence expirée' : 'Activez votre'}
                  <br />
                  {isExpired ? '— contactez l\'admin' : 'licence'}
                </h2>
                <p className="mt-1 text-[13px] leading-snug" style={{ color: mutedColor }}>
        ...
        {/* RIGHT PANEL - FOTSY */}
        <div className="flex w-full flex-col gap-3 p-5 pt-12 lg:w-1/2" style={{ background: rightBoxBg }}>
          <div className="flex items-center gap-2 lg:hidden">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ background: 'linear-gradient(135deg, #4F46E5, #4338CA)' }}>
              <Cloud className="h-4 w-4 text-white" />
            </div>
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-[14px]" style={{ color: mutedColor }}>
                {isExpired
          ...
          {showSuccess && successMessage && (
            <div className="flex items-start gap-3 rounded-lg border p-2.5" style={{ background: 'rgba(16,185,129,0.1)', borderColor: 'rgba(16,185,129,0.3)' }}>
              <CheckCircle2 className="h-5 w-5 mt-0.5 shrink-0" style={{ color: '#10B981' }} />
              <div>
                <p className="text-[14px] font-semibold leading-tight" style={{ color: '#10B981' }}>Succès</p>
                <p className="text-[14px] leading-snug" style={{ color: mutedColor }}>{successMessage}</p>
              </div>
            </div>
          )}

          {isExpired && expirationDate && (
            <div className="flex items-center justify-between rounded-lg border p-2" style={{ background: 'rgba(239,68,68,0.1)', borderColor: 'rgba(239,68,68,0.3)' }}>
              <div className="flex items-center gap-2">
                <XCircle className="h-3.5 w-3.5" style={{ color: '#EF4444' }} />
                <span className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>Date d'expiration :</span>
              </div>
              <span className="text-[13px] font-semibold leading-tight" style={{ color: '#EF4444' }}>
          ...
          {!showSuccess ? (
            <form onSubmit={handleActivate} className="space-y-2">
              ...
              <button
                type="submit"
                disabled={isLoading || licenseLoading}
                className="group relative flex h-10 w-full items-center justify-center gap-2 rounded-lg text-[15px] font-semibold text-white transition-all hover:opacity-95 disabled:opacity-50 bg-brand-500 hover:bg-brand-600"
                style={{ boxShadow: `0 4px 12px ${primaryBg}` }}
              >
                {isLoading || licenseLoading ? (
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[{ icon: Zap, text: 'Instantané' }, { icon: ShieldCheck, text: 'Sécurisé' }, { icon: Cloud, text: 'Offline' }].map((item, i) => (
              <div key={i} className="flex items-center justify-center gap-1.5 rounded-lg border p-2" style={{ borderColor }}>
                <item.icon size={13} style={{ color: licenseBadges[i % 4].color }} />
                <span className="text-[13px] font-medium leading-tight" style={{ color: mutedColor }}>{item.text}</span>
              </div>
            ))}
          </div>

          <div className="flex items-center justify-center gap-4">
            <button type="button" onClick={() => navigate('/support')} className="group flex items-center gap-1.5 text-[13px] font-medium leading-tight hover:underline" style={{ color: '#4F46E5' }}>
              <LifeBuoy className="h-3.5 w-3.5" /> Support
            </button>
            <button type="button" onClick={() => navigate('/buy-license')} className="group flex items-center gap-1.5 text-[13px] font-medium leading-tight hover:underline" style={{ color: '#4F46E5' }}>
              <CreditCard className="h-3.5 w-3.5" /> Acheter
            </button>
          </div>
          ...
          <div className="flex items-center justify-between border-t pt-2 mt-auto" style={{ borderColor }}>
            ...
            <div className="flex gap-3">
              <Link to="/terms" className="text-[13px] leading-tight hover:underline" style={{ color: isDark ? '#64748B' : '#94A3B8' }}>Conditions</Link>
              <Link to="/support" className="text-[13px] leading-tight hover:underline" style={{ color: isDark ? '#64748B' : '#94A3B8' }}>Support</Link>
            </div>
          </div>
        </div>
      </div>
