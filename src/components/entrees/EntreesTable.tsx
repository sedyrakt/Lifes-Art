...
const EntreesTable: React.FC<EntreesTableProps> = ({
  ...
  const toggleMenu = (id: number, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const MENU_WIDTH = 180, MENU_HEIGHT = 80, MENU_PADDING = 12;
  ...
  if (safeEntrees.length === 0) {
    return (
      <div className={`flex min-h-[270px] flex-col items-center justify-center overflow-hidden rounded-xl border-[0.5px] px-6 py-10 text-center shadow-sm ${tableBackground} ${borderColor} dark:shadow-[0_14px_40px_rgba(0,0,0,0.20)]`}>
        <h3 className="text-[15.5px] font-semibold tracking-[-0.01em] text-slate-900 dark:text-slate-100">Aucune entrée</h3>
        <p className="mt-2 max-w-[390px] text-[14.5px] leading-6 text-slate-500 dark:text-slate-400">Aucune entrée de stock n'a été enregistrée pour le moment.</p>
      </div>
    );
  }
      ...
      {safeSelectedIds.size > 0 && (
        <div className={`flex flex-wrap items-center justify-between gap-2 border-b px-3.5 py-2 ${isDark ? 'border-white/[0.08] bg-brand-500/[0.065]' : 'border-slate-200 bg-brand-50'}`}>
          ...
          <div className="flex items-center gap-1.5">
            <button type="button" onClick={onBulkDelete} className="rounded-lg bg-danger-500 px-2.5 py-1.5 text-[13.5px] font-semibold text-white shadow-sm transition hover:bg-danger-600 active:scale-[0.98]">
              Supprimer
            </button>
            <button type="button" onClick={onClearSelection} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[13.5px] font-semibold text-slate-600 shadow-sm transition hover:bg-slate-50 dark:border-white/[0.12] dark:bg-[#0F172A] dark:text-slate-300 dark:hover:bg-slate-700">
              Désélectionner
            </button>
          </div>
        </div>
        ...
        <table className={`w-full min-w-[850px] table-fixed border-collapse text-left ${borderColor}`}>
          ...
          <thead className={`sticky top-0 z-20 backdrop-blur-xl ${isDark ? 'bg-[#0F172A]/97' : 'bg-slate-50'}`}>
            <tr className="text-[13px] font-semibold uppercase tracking-[0.055em] text-slate-500 dark:text-slate-400">
              ...
              <th className={`w-[120px] border-b px-2 py-2.5 align-middle ${headerBorderColor}`}>Référence</th>
              <th className={`w-[220px] border-b px-2 py-2.5 align-middle ${headerBorderColor}`}>Produit</th>
              <th className={`w-[90px] border-b px-2 py-2.5 align-middle ${headerBorderColor}`}>Quantité</th>
              <th className={`w-[150px] border-b px-2 py-2.5 align-middle ${headerBorderColor}`}>Prix unitaire (Ar)</th>
              <th className={`w-[60px] border-b px-2 py-2.5 text-right align-middle ${headerBorderColor}`}>Actions</th>
            </tr>
          </thead>
          <tbody className={tableBackground}>
            {safeEntrees.map((entree, index) => {
              ...
              return (
                <tr key={entree.id} className={`group h-[60px] cursor-pointer transition-colors duration-150 ${isSelected ? (isDark ? 'bg-brand-500/[0.085]' : 'bg-brand-50') : isDark ? 'hover:bg-white/[0.025]' : 'hover:bg-slate-50'} ${firstRowShadow}`}>
                  ...
                  <td className={`border-b px-2 py-2 align-middle ${cellBorderColor}`}>
                    <span className="inline-flex min-w-[40px] items-center justify-center rounded-md border border-success-200 bg-success-50 px-1.5 py-0.5 text-[13.5px] font-semibold text-success-700 dark:border-success-500/25 dark:bg-success-500/10 dark:text-success-400">
                  ...
                  <td className={`border-b px-1.5 py-2 align-middle text-right ${cellBorderColor}`} onClick={(e) => e.stopPropagation()}>
                    <button type="button" title="Actions" aria-label={`Actions pour ${reference}`} onClick={(e) => toggleMenu(entree.id, e)} className={`flex h-7 w-7 items-center justify-center rounded-md border border-transparent text-slate-400 transition-all duration-150 ${openMenuId === entree.id ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'hover:border-slate-200 hover:bg-slate-50 hover:text-brand-600 dark:hover:border-white/[0.12] dark:hover:bg-slate-800 dark:hover:text-slate-200'}`}>
