...
const EntreesHeader: React.FC<EntreesHeaderProps> = ({ onAddEntree, onExport, refreshing = false, onRefresh, totalItems }) => {
  ...
  return (
    <header className="relative z-[100] mb-4 w-full overflow-visible">
      <div className={`relative flex flex-col gap-3 rounded-xl border-[0.5px] shadow-sm transition-colors md:flex-row md:items-center md:justify-between px-3 py-2.5 ${isDark ? 'bg-[#0F172A]' : 'bg-white'} ${borderColor}`}>
        ...
        <div className="relative z-10 flex shrink-0 items-center gap-2">
                      ...
                      <div className="grid grid-cols-1 gap-1">
                        {PERIOD_OPTIONS.map((opt) => (
                          <button
                            ...
                            className={`flex items-center gap-2 rounded-md px-2 py-1.5 text-left text-[14px] font-medium transition-colors ${selectedPeriod === opt.value ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-700/30'}`}
                          >
                            <span className={`flex h-3.5 w-3.5 items-center justify-center rounded-full border ${selectedPeriod === opt.value ? 'border-brand-500 bg-brand-500' : 'border-slate-300 dark:border-slate-600'}`}>
          ...
          <button
            ...
            onClick={onAddEntree}
            /* ⭐ Button : 13px → 14px */
            className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-brand-500 px-3.5 text-[14px] font-semibold text-white shadow-sm transition-colors hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500/30 active:scale-[0.98]"
          >
            {/* ⭐ Icon : 16 → 17 */}
