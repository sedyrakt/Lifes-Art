...
const EntreesHeader: React.FC<EntreesHeaderProps> = ({ onAddEntree, onExport, refreshing = false, onRefresh, totalItems }) => {
  ...
  return (
    <header className="relative z-[100] mb-4 w-full overflow-visible rounded-xl border bg-white px-4 py-3.5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] dark:bg-slate-900 dark:border-white/[0.12]">
      ...
      <div className="relative z-10 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {onRefresh && (
            <button type="button" onClick={onRefresh} disabled={refreshing} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border text-slate-500 transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-brand-600 dark:border-white/[0.12] dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800">
              <RefreshCw size={16} className={refreshing ? 'animate-spin' : ''} />
            </button>
          )}
          {onExport && (
            <div className="relative z-[300]" ref={exportMenuRef}>
              <button type="button" onClick={() => setShowExportMenu(prev => !prev)} aria-expanded={showExportMenu} aria-haspopup="menu" className="inline-flex h-9 items-center gap-2 rounded-lg bg-brand-500 px-4 text-[13px] font-semibold text-white shadow-sm transition-all hover:bg-brand-600 hover:shadow-md active:scale-[0.98] dark:bg-brand-500 dark:hover:bg-brand-600">
              ...
              {showExportMenu && (
                <div role="menu" className={`absolute right-0 top-full mt-2 w-[520px] overflow-hidden rounded-xl border ${borderColor} ${cardBg} py-1.5 shadow-[0_12px_30px_rgba(0,0,0,0.12)] animate-in fade-in zoom-in-95 duration-100 dark:shadow-2xl`}>
                    ...
                    <div className="w-1/2 border-r border-slate-200 p-3 dark:border-white/[0.08]">
                      ...
                      <div className="grid grid-cols-1 gap-1">
                        {PERIOD_OPTIONS.map((opt) => (
                          <button key={opt.value} type="button" onClick={() => setSelectedPeriod(opt.value)} className={`flex items-center gap-2 rounded-md px-2 py-1.5 text-left text-[14px] font-medium transition-colors ${selectedPeriod === opt.value ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-700/30'}`}>
                            <span className={`flex h-4 w-4 items-center justify-center rounded-full border ${selectedPeriod === opt.value ? 'border-brand-500 bg-brand-500' : 'border-slate-300 dark:border-slate-600'}`}>
                      ...
                      {selectedPeriod === 'custom' && (
                        <div className="mt-2 flex items-center gap-2 rounded-md border border-slate-200 px-2 py-1.5 dark:border-white/[0.1]">
                          ...
                          <input type="date" value={selectedCustomDate} onChange={(e) => setSelectedCustomDate(e.target.value)} className="w-full bg-transparent text-[14px] font-medium text-slate-700 outline-none dark:text-slate-200" />
                        </div>
                      )}
                    </div>

                    {/* RIGHT BOX: Format export */}
                  ...
                  <div className="mt-1 border-t border-slate-200 px-3 py-2 dark:border-white/[0.08]">
                    <button type="button" onClick={() => setShowExportMenu(false)} className="flex w-full items-center justify-center gap-1.5 rounded-md py-1.5 text-[13px] font-medium text-slate-400 transition-colors hover:bg-brand-50 hover:text-brand-600 dark:hover:bg-slate-700/30 dark:hover:text-slate-300">
          ...
          <button type="button" onClick={onAddEntree} className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-brand-500 px-3.5 text-[13px] font-semibold text-white shadow-sm transition-all hover:bg-brand-600 hover:shadow-md">
