        ...
        <div className="relative min-w-[100px] flex-1">
          ...
          {searchTerm && (
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
        ...
        <div className="flex shrink-0 items-center gap-2">
          <div className="relative shrink-0">
            <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2.5">
              <ArrowUpDown size={14} strokeWidth={2} className="text-brand-500 dark:text-brand-500" />
            </div>
            <select
              value={sortOption}
              onChange={(e) => onSortChange(e.target.value)}
              className={`h-10 min-w-[110px] cursor-pointer appearance-none rounded-lg border ${borderColor} ${backgroundColor} pl-8 pr-7 text-[14px] ${textColor} outline-none shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-all duration-150 ${hoverBorderColor} ${!isDark ? 'hover:bg-slate-50' : 'dark:hover:bg-slate-800/60'} focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10`}
