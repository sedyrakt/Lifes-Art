...
// ⭐ Bordure gauche indigo ajoutée (comme les autres headers)
...
const CommandesHeader: React.FC<CommandesHeaderProps> = ({
  ...
  return (
    <header className="mb-4 w-full relative z-[100]">
      <div className={`group relative flex flex-col gap-3 overflow-visible rounded-xl border-[0.5px] shadow-sm transition-colors duration-200 md:flex-row md:items-center md:justify-between px-3 py-2 ${isDark ? 'bg-[#0F172A]' : 'bg-white'} ${borderColor}`}>
        ...
        <div className="relative z-10 flex min-w-0 flex-col">
          <div className="flex items-center gap-2">
            <h1 className="text-[18px] font-semibold leading-tight tracking-[-0.02em] text-slate-900 dark:text-slate-100">Commandes</h1>
            {totalItems > 0 && (
              <span className="inline-flex min-w-[24px] items-center justify-center rounded-md border border-brand-200 bg-brand-50 px-1.5 py-0.5 text-[11.5px] font-semibold leading-tight text-brand-600 dark:border-brand-500/25 dark:bg-brand-500/10 dark:text-brand-400">
                {totalItems}
              </span>
            )}
          </div>
          <p className="mt-0.5 text-[12.5px] font-medium leading-tight text-slate-500 dark:text-slate-400">Suivez et gérez vos commandes en temps réel.</p>
        </div>
        ...
        <div className="relative z-10 flex w-full shrink-0 items-center gap-2 md:w-auto">
          ...
          <div className="relative z-[200]" ref={exportMenuRef}>
            <button
              ...
              aria-haspopup="menu"
              className="flex h-9 items-center gap-2 rounded-lg bg-brand-500 px-3.5 text-[13px] font-semibold text-white shadow-sm transition-colors hover:bg-brand-600 active:scale-[0.98]"
            >
          ...
          <button
            ...
            className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-brand-500 px-3.5 text-[13px] font-semibold text-white shadow-sm transition-colors hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500/30 active:scale-[0.98]"
          >
