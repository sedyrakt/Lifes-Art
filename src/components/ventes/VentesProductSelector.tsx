...
const COLORS = {
  ...
  dark: {
    ...
    primary: '#4F46E5', soft: '#0F172A', bg: '#0F172A',
...
const VentesProductSelector: React.FC<Props> = ({ produits, selectedProduits, onAddProduit, onUpdateQuantite, onRemoveProduit, onClearPanier, isDark }) => {
  ...
  const add = () => {
    ...
    else onAddProduit(id, qty, product.prix_vente, tvaRate);
    ...
    <div className="p-4 space-y-3.5">
              ...
              return (
                ...
                >
                  <div className="min-w-0 flex-1">
                    {/* ⭐ Name : text-[14px] → text-[15px] */}
                    <p className="text-[15px] font-semibold truncate" style={{ color: isRupture ? theme.muted : theme.text }}>{p.nom}</p>
                    {isRupture ? (
                      /* ⭐ Rupture : text-[13px] → text-[14px] */
                      <p className="text-[14px] font-bold" style={{ color: theme.red }}>Rupture stock</p>
                    ) : (
                      /* ⭐ Code+TVA : text-[14px] → text-[14.5px] */
                      <p className="text-[14.5px] truncate" style={{ color: theme.muted }}>
                        {p.code} · TVA {((Number.isFinite(Number(p.tva_rate)) ? Number(p.tva_rate) : 0.2) * 100).toFixed(0)}%
                      </p>
                    )}
                  </div>
                  {/* ⭐ Price : text-[14px] → text-[15px] */}
                  <span className="text-[15px] font-semibold" style={{ color: isRupture ? theme.red : theme.primary }}>{p.prix_vente.toLocaleString('fr-FR')} Ar</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="border rounded-xl p-3.5 space-y-3" style={{ borderColor: theme.primary, background: theme.soft }}>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-right shrink-0">
              {/* ⭐ Price : text-[14px] → text-[15px] */}
              <p className="text-[15px] font-bold" style={{ color: theme.primary }}>{selectedProduct.prix_vente.toLocaleString('fr-FR')} Ar</p>
              {/* ⭐ Stock : text-[11px] → text-[12.5px] */}
              <p className="text-[12.5px] text-slate-500">Stock: {selectedProduct.quantite_stock}</p>
            </div>
            {/* ⭐ Close : size 15 → 17, p-1 → p-1.5 */}
          ...
          <div className="flex gap-2">
            {/* ⭐ Qty input : w-20 → w-24, h-11, text-[15px] → text-[15.5px] */}
            ...
            {/* ⭐ Add button : h-11, text-[14px] → text-[15px], icon 15 → 17 */}
      ...
      {selectedProduits.length === 0 ? (
        ...
        <div className="space-y-2.5">
            ...
            return (
              ...
              <div key={item.id} className="flex items-center justify-between gap-3 border rounded-lg px-3.5 py-2.5" style={{ borderColor: theme.border }}>
                <div className="min-w-0 flex-1">
                  ...
                  {/* ⭐ Info : text-[14px] → text-[14px] */}
                  <p className="text-[14px]" style={{ color: theme.muted }}>{p.prix_vente.toLocaleString('fr-FR')} Ar · TVA {(tvaRate * 100).toFixed(0)}%</p>
                </div>
                <div className="flex items-center gap-1">
                  {/* ⭐ Minus : w-8 h-8 → w-9 h-9, icon 13 → 14 */}
                ...
                {/* ⭐ Line total : text-[15px] → text-[15.5px] */}
          ...
          {/* ⭐ Footer : pt-3, label 13px → 13.5px, value 15px → 15.5px */}
          ...
          {selectedProduits.length > 0 && (
            /* ⭐ Clear : text-[14px] → text-[14.5px], icon 13 → 15 */
