...
// ⭐ TYPOGRAPHIE alignée sur DashboardHeader
// ⭐ FONT SIZE: h1 20px, subtitle 13px, buttons 14px, menu items 14px

import React, { useState, useRef, useEffect } from 'react';
import { Download, RefreshCw, FileSpreadsheet, Plus, FileText, File as FileCsv, ChevronDown, X, Check, Calendar } from 'lucide-react';
import { useTheme } from '../../contexts/ThemeContext';
import { ExportPeriod } from '../../hooks/useProduitsData';
      ...
      <div className={`group relative flex flex-col gap-3 rounded-xl border-[0.5px] shadow-sm transition-colors duration-200 md:flex-row md:items-center md:justify-between px-3 py-2.5 ${isDark ? 'bg-[#0F172A]' : 'bg-white'} ${borderColor}`}>
        ...
        <div className="relative z-10 flex min-w-0 flex-col">
          <div className="flex items-center gap-2">
            {/* ⭐ h1 : 18px → 20px */}
            <h1 className="text-[20px] font-semibold leading-tight tracking-[-0.02em] text-slate-900 dark:text-slate-100">Produits</h1>
          ...
          <p className="mt-0.5 text-[13px] font-medium leading-tight text-slate-500 dark:text-slate-400">Gérez et organisez votre catalogue produits.</p>
        </div>
