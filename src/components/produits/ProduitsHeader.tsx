...
const ProduitsHeader: React.FC<ProduitsHeaderProps> = ({
  ...
  return (
    <header className="mb-4 w-full">
      <div className="group relative flex flex-col gap-3 overflow-hidden rounded-xl border bg-white px-4 py-3.5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-all duration-200 md:flex-row md:items-center md:justify-between dark:bg-slate-900" style={{ borderColor: isDark ? 'rgba(255,255,255,0.15)' : '#E2E8F0' }}>
        ...
        <div className="relative z-10 flex w-full shrink-0 items-center gap-2 md:w-auto">
          ...
          {onRefresh && (
            <button
              ...
              onClick={onRefresh}
              disabled={refreshing}
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border px-3 text-[14px] font-medium text-slate-500 transition-all duration-150 hover:border-brand-500/20 hover:bg-brand-50 hover:text-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500/20 disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-200"
              style={{ borderColor: isDark ? 'rgba(255,255,255,0.15)' : '#E2E8F0', background: isDark ? '#1E293B' : '#FFFFFF' }}
