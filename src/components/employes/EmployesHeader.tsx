...
import { ExportPeriod } from '../../hooks/useEmployesData';
...
const EmployesHeader: React.FC<EmployesHeaderProps> = ({
  onAddEmploye, onExport, refreshing = false, onRefresh, totalItems,
}) => {
  ...
  // ⭐ FIX: FontSize 14px ho an'ny export items
  ...
  return (
    <header className="mb-4 w-full relative z-[100]">
      <div className="group relative flex flex-col gap-3 rounded-xl border bg-white px-4 py-3.5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-all duration-200 md:flex-row md:items-center md:justify-between dark:bg-[#0F172A]" style={{ borderColor: isDark ? 'rgba(255,255,255,0.12)' : '#E2E8F0' }}>
        ...
        <div className="relative z-10 flex w-full shrink-0 items-center gap-2 md:w-auto">
          ...
          {/* ⭐ Menu Exporter avec période + date picker (FLEXBOX 2 COLONNES) */}
          <div className="relative z-[200]" ref={exportMenuRef}>
            ...
            {showExportMenu && (
              <div
                role="menu"
                className={`absolute right-0 top-full mt-2 w-[520px] overflow-hidden rounded-xl border ${borderColor} ${cardBg} py-1.5 shadow-[0_12px_30px_rgba(0,0,0,0.12)] animate-in fade-in zoom-in-95 duration-100 dark:shadow-2xl`}
              >
                {/* Header */}
                ...
                <div className="flex w-full">
                  {/* LEFT BOX: Période */}
                  <div className="w-[50%] border-r border-slate-200 p-3 dark:border-white/[0.08]">
                  ...
                  <div className="w-[50%] p-2.5">
                    ...
                    <button type="button" role="menuitem" onClick={() => { onExport('excel', selectedPeriod, selectedDate); setShowExportMenu(false); }} className={exportItemClass}>
                    ...
                    <button type="button" role="menuitem" onClick={() => { onExport('pdf', selectedPeriod, selectedDate); setShowExportMenu(false); }} className={exportItemClass}>
                    ...
                    <button type="button" role="menuitem" onClick={() => { onExport('csv', selectedPeriod, selectedDate); setShowExportMenu(false); }} className={exportItemClass}>
