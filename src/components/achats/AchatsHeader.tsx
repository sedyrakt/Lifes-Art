...
import { ExportPeriod } from '../../hooks/useAchatsData';
...
const AchatsHeader: React.FC<AchatsHeaderProps> = ({
  onAddAchat, onExport, refreshing = false, onRefresh, totalItems,
}) => {
      ...
      <div className={`group relative flex flex-col gap-3 rounded-xl border-[0.5px] shadow-sm transition-colors duration-200 md:flex-row md:items-center md:justify-between px-3 py-2 ${isDark ? 'bg-[#0F172A]' : 'bg-white'} ${borderColor}`}>
        ...
        <div className="relative z-10 flex min-w-0 flex-col">
          <div className="flex items-center gap-2">
            <h1 className="text-[18px] font-semibold leading-tight tracking-[-0.02em] text-slate-900 dark:text-slate-100">Achats</h1>
          ...
          <p className="mt-0.5 text-[12.5px] font-medium leading-tight text-slate-500 dark:text-slate-400">Gérez vos achats de produits auprès de vos fournisseurs.</p>
        </div>
