// electron/ipc/products/index.cjs - IPC HANDLERS PRODUITS
// ⭐ FIX: AUTO "inactif" raha rupture (stock <= 0)
// ⭐ FIX: NESORINA NY IMAGE - TSY AMPIASAINA
// ⭐ FIX: NAMPIANA NY TVA RATE (0%, 10%, 20%)
...
function registerProductsHandlers(ipcMain) {
  ...
  ipcMain.handle('products:get-by-categorie', withDbCheck((db, event, categorieId, limit = 50) => {
    ...
    // ⭐ FIX: NAMPIANA NY p.tva_rate
    const data = db.prepare(`SELECT p.id, p.code, p.nom, p.prix_vente, p.quantite_stock, p.unite, p.tva_rate, c.nom AS categorie_nom FROM produits p LEFT JOIN categories c ON c.id = p.categorie_id WHERE p.categorie_id = ? AND p.status = 'actif' ORDER BY p.nom COLLATE NOCASE ASC, p.id ASC LIMIT ?`).all(safeCategorieId, safeLimit);
