// electron/ipc/products/handlers.cjs
'use strict';
...
function emitProductsChanged(productData) {
  try {
    ...
    for (const win of windows) {
      ...
      try {
        win.webContents.send('products:changed', productData);
      } catch (err) {}
    }
  } catch (err) {}
}
...
function registerProductsHandlers(ipcMain) {
  ...
  const channels = [
    ...
    'products:update',
    'products:delete',
    'products:get-alertes',
    'products:get-top',
    'products:get-by-categorie',
    'products:search',
  ...
  ipcMain.handle('products:create', withDbCheck((db, event, data = {}) => {
    ...
    `).get(
      ...
      numberOr(data.prix_achat, 0),
      prixVente,
      stock,
      Math.max(0, Number.parseInt(data.quantite_minimale, 10) || 5),
      String(data.unite || 'pièce').trim(),
      finalStatus,
  ...
  ipcMain.handle('products:get-alertes', withDbCheck((db, event, limit = 100) => {
    ...
    return {
      ...
      data: db.prepare(`
        ...
        ORDER BY quantite_stock ASC, id ASC
        LIMIT ?
      `).all(safeLimit)
    };
  }));
  ...
  ipcMain.handle('products:search', withDbCheck((db, event, term, limit = 50) => {
    ...
    return {
      ...
      data: db.prepare(`
        ...
        ORDER BY p.nom COLLATE NOCASE ASC, p.id ASC
        LIMIT ?
      `).all(terms, safeLimit),
  ...
  ipcMain.handle('products:update-stock', withDbCheck((db, event, id, quantite, type, userId = null) => {
    ...
    db.prepare(`
      UPDATE produits
      SET quantite_stock = ?,
        status = CASE WHEN ? <= 0 THEN 'inactif' ELSE status END,
        updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(nouveauStock, nouveauStock, productId);
