...
function prepareStatements() {
  try {
    const db = getDb();
    if (!db || !db.open) { error('❌ [ventes.statements] DB indisponible'); return false; }
    ...
    statements = {
      ...
      // ⭐ Manampy mode_paiement + modalite_paiement + frais_livraison + date_limite_paiement
      stmtCreateDevis: db.prepare(`
        INSERT INTO devis (
          client_id, client_nom, reference, total_ht, total_ttc,
          statut_paiement, montant_paye, montant_restant, validite_jours, observation,
      ...
      stmtCreateFacture: db.prepare(`
        INSERT INTO factures (
          client_id, client_nom, reference, total_ht, total_ttc,
          statut_paiement, montant_paye, montant_restant, observation,
          mode_paiement, modalite_paiement, frais_livraison, date_limite_paiement
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `),
      ...
      stmtUpdateDevis: db.prepare(`
        UPDATE devis
        SET client_id = ?, client_nom = ?, reference = ?, total_ht = ?, total_ttc = ?,
            statut_paiement = ?, montant_paye = ?, montant_restant = ?, validite_jours = ?,
            observation = ?, mode_paiement = ?, modalite_paiement = ?, frais_livraison = ?,
      ...
      stmtUpdateFacture: db.prepare(`
        ...
        SET client_id = ?, client_nom = ?, reference = ?, total_ht = ?, total_ttc = ?,
            statut_paiement = ?, montant_paye = ?, montant_restant = ?, observation = ?,
            mode_paiement = ?, modalite_paiement = ?, frais_livraison = ?,
            date_limite_paiement = ?
        WHERE id = ?
      `),
    ...
    log('✅ [ventes.statements] Statements préparés (mode/modalité/frais livraison)');
    return true;
  } catch (err) {
